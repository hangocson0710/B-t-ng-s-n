                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                                                @if($item->is_active == 0 || $item->is_active == 2)
                                                    <a class="dropdown-item browse" data-id="{{$item->id}}" href="javascript:void(0)"><i class="dw dw-eye"></i> Duyệt</a>
                                                    <a class="dropdown-item no-browse" data-id="{{$item->id}}" href="javascript:void(0)"><i class="icon-copy dw dw-ban"></i> Không duyệt</a>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
...
@section('Script')
    ...
    <script>
        $('.browse').click(function (){
            ...
            Swal.fire({
                title: 'Duyệt dự án',
                text: "Sau khi đồng ý sẽ duyệt dự án này",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href='{{route('admin.project.browse', '')}}/' + id;
                }
            })
        });
        $('.no-browse').click(function (){
            var id = $(this).data('id');
            Swal.fire({
                title: 'Không duyệt dự án',
                text: "Đồng ý sẽ không duyệt dự án",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    window.location.href='{{route('admin.project.no_browse', '')}}/' + id;
                }
            })
        });
    </script>
